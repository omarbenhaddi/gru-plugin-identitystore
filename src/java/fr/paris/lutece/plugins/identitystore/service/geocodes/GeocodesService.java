...
public class GeocodesService {
     ...
     */
    public static void processCountryForUpdate(final Identity identity, final List<CertifiedAttribute> attrToCreate, final List<CertifiedAttribute> attrToUpdate,
                                         final String clientCode, final IdentityChangeResponse response ) throws IdentityStoreException
    {
        final CertifiedAttribute countryCodeToCreate = attrToCreate.stream( ).filter( a -> a.getKey( ).equals(Constants.PARAM_BIRTH_COUNTRY_CODE ) ).findFirst( )
                .orElse( null );
        if ( countryCodeToCreate != null )
        {
            attrToCreate.remove( countryCodeToCreate );
        }
        CertifiedAttribute countryLabelToCreate = attrToCreate.stream( ).filter( a -> a.getKey( ).equals( Constants.PARAM_BIRTH_COUNTRY ) ).findFirst( ).orElse( null );
        if ( countryLabelToCreate != null )
        {
            attrToCreate.remove( countryLabelToCreate );
        }
        final CertifiedAttribute countryCodeToUpdate = attrToUpdate.stream( ).filter( a -> a.getKey( ).equals( Constants.PARAM_BIRTH_COUNTRY_CODE ) ).findFirst( )
                .orElse( null );
        if ( countryCodeToUpdate != null )
        {
            attrToUpdate.remove( countryCodeToUpdate );
        }
        CertifiedAttribute countryLabelToUpdate = attrToUpdate.stream( ).filter( a -> a.getKey( ).equals( Constants.PARAM_BIRTH_COUNTRY ) ).findFirst( ).orElse( null );
                                       ...
                                       final String clientCode, final IdentityChangeResponse response ) throws IdentityStoreException
    {
        ...
        CertifiedAttribute cityLabelToCreate = attrToCreate.stream( ).filter( a -> a.getKey( ).equals( Constants.PARAM_BIRTH_PLACE ) ).findFirst( ).orElse( null );
        ...
        CertifiedAttribute cityLabelToUpdate = attrToUpdate.stream( ).filter( a -> a.getKey( ).equals( Constants.PARAM_BIRTH_PLACE ) ).findFirst( ).orElse( null );
                                       ...
                                       final IdentityChangeResponse response ) throws IdentityStoreException
    {
        ...
        CertifiedAttribute cityCodeToCreate = attrToCreate.stream( ).filter( a -> a.getKey( ).equals( Constants.PARAM_BIRTH_PLACE_CODE ) ).findFirst( ).orElse( null );
        if ( cityCodeToCreate != null )
        {
            attrToCreate.remove( cityCodeToCreate );
        }
